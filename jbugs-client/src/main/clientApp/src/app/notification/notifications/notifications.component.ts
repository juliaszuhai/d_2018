...
export class NotificationsComponent implements OnInit {
  ...
  getTypeOfNotification(notificationType) {
    ...
    if (notificationType == 'WELCOME_NEW_USER') {
      this.translate.get('notifications.welcome').subscribe((res: string) => notification = res);
    } else if (notificationType == 'USER_UPDATED') {
      this.translate.get('notifications.userUpdate').subscribe((res: string) => notification = res);
    } else if (notificationType == 'BUG_CREATED') {
      this.translate.get('notifications.bugUpdate').subscribe((res: string) => notification = res);
    } else if (notificationType == 'BUG_UPDATED') {
      this.translate.get('notifications.bugUpdate').subscribe((res: string) => notification = res);
    } else if (notificationType == 'BUG_CLOSED') {
      this.translate.get('notifications.bugClosed').subscribe((res: string) => notification = res);
    } else if (notificationType == 'BUG_STATUS_UPDATED') {
      this.translate.get('notifications.bugStatusUpdate').subscribe((res: string) => notification = res);
    } else if (notificationType == 'USER_DEACTIVATED') {
      this.translate.get('notifications.userDeactivate').subscribe((res: string) => notification = res);
    }
    return notification;
  }
  ...
  getTitle(notificationData: NotificationData) {
    ...
    if (notificationData.typeNotification == 'WELCOME_NEW_USER') {
      ...
      if (newData.username === this.username) {
        this.translate.get('notifications.userUpdatedByMe').subscribe((res: string) => notificationTitle = res);
      } else {
        this.translate.get('notifications.userUpdatedBySomeone').subscribe((res: string) => notificationTitle = res);
      }
    } else if (notificationData.typeNotification == 'BUG_CREATED') {
      this.translate.get('notifications.bugTitleCreated').subscribe((res: string) => notificationTitle = res);

    } else if (notificationData.typeNotification == 'BUG_UPDATED') {
      this.translate.get('notifications.bugTitleUpdated').subscribe((res: string) => notificationTitle = res);

    } else if (notificationData.typeNotification == 'BUG_CLOSED') {
      this.translate.get('notifications.bug').subscribe((res: string) => notificationTitle = res);
      notificationTitle = notificationTitle + newData.title;
      this.translate.get('notifications.titleBugClosed').subscribe((res: string) => titleBug = res);
      notificationTitle = notificationTitle + titleBug;
    } else if (notificationData.typeNotification == 'BUG_STATUS_UPDATED') {
      this.translate.get('notifications.status').subscribe((res: string) => notificationTitle = res);
      notificationTitle = notificationTitle + newData.title;
      this.translate.get('notifications.titleStatus').subscribe((res: string) => titleBugStatus = res);
      notificationTitle = notificationTitle + titleBugStatus;
    } else if (notificationData.typeNotification == 'USER_DEACTIVATED') {
      this.translate.get('notifications.user').subscribe((res: string) => notificationTitle = res);
      notificationTitle = notificationTitle + newData.firstName;
      let messageTitle = '';
      this.translate.get('notifications.titleUserDeactivate').subscribe((res: string) => messageTitle = res);
  ...
  getContentMessage(notificationData: NotificationData) {
    let contentMessage = '';
    if (notificationData.typeNotification == 'WELCOME_NEW_USER') {
      this.translate.get('notifications.contentMessage').subscribe((res: string) => contentMessage = res);
    } else if (notificationData.typeNotification == 'USER_UPDATED') {
      this.translate.get('notifications.userUpdateContent').subscribe((res: string) => contentMessage = res);
    } else if (notificationData.typeNotification == 'BUG_CREATED') {
      this.translate.get('notifications.bugContentMessage').subscribe((res: string) => contentMessage = res);
    } else if (notificationData.typeNotification == 'BUG_UPDATED') {
      this.translate.get('notifications.bugContentMessage').subscribe((res: string) => contentMessage = res);
    } else if (notificationData.typeNotification == 'BUG_CLOSED') {
      this.translate.get('notifications.bugContentMessage').subscribe((res: string) => contentMessage = res);
      ...
      this.translate.get('notifications.userDeactivateContent').subscribe((res: string) => contentMessage = res);
    }
  ...
  getMessageUpdateUser(notification: NotificationData) {
    ...
    if (this.messageNotification.newUser.username === this.username) {
      this.translate.get('notifications.userUpdatedByMe').subscribe((res: string) => this.messageNotification.subtitleMessage = res);
    } else {
      this.translate.get('notifications.userUpdatedBySomeone').subscribe((res: string) => this.messageNotification.subtitleMessage = res);
    }

    this.translate.get('notifications.userUpdateContent').subscribe((res: string) => this.messageNotification.content = res);
  ...
  getMessage(notification: NotificationData) {
    ...
    if (notification.typeNotification == 'WELCOME_NEW_USER') {
      ...
      this.translate.get('notifications.bugUpdate').subscribe((res: string) => message = res);
    } else if (notification.typeNotification == 'BUG_CREATED') {
      this.translate.get('notifications.bugCreated').subscribe((res: string) => message = res);
    } else if (notification.typeNotification == 'BUG_CLOSED') {
      this.translate.get('notifications.bugClosed').subscribe((res: string) => message = res);
    } else if (notification.typeNotification == 'BUG_STATUS_UPDATED') {
      this.translate.get('notifications.bugStatusUpdate').subscribe((res: string) => message = res);
    } else if (notification.typeNotification == 'USER_DEACTIVATED') {
      this.translate.get('notifications.userDeactivate').subscribe((res: string) => message = res);
    }
  }
